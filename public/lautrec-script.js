...
async function pollBag(){
  ...
  try{
    ...
    // só aplica se for realmente mais novo — evita que uma resposta atrasada (fetch que já estava
    // a caminho antes de um delete/edição local) sobrescreva uma mudança mais recente já salva
    if(incoming>known){
      bagUpdatedAt=rows[0].updated_at;
      renderBagItems(rows[0].data?.items||[]);
    }
